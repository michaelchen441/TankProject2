...
public class Menu
{
	...
	void draw(Graphics g, ResourceLibrary l, int inSurvivalHighScore, int inClassicHighScore, boolean soundOn)
	{
		...
		// draw title
		g.setFont(new Font("TimesRoman", Font.PLAIN, 175)); // Times New Roman
															// font; size 200
		g.setColor(Color.BLACK); // Black colored text
		g.drawString("TANK GAME", titleX, titleY); // Constructs the text and
													// draws it on panel
	...
	{ // to start classic game
		if ((x >= buttonX && x <= buttonX + buttonWidth) && (y >= button1Y && y <= button1Y + buttonHeight))
		{
			return true;
		}
		return false;
	...
	public boolean clickedButton2(int x, int y)
	{ // to start survival mode
		if ((x >= buttonX && x <= buttonX + buttonWidth) && (y >= button2Y && y <= button2Y + buttonHeight))
		{
			return true;
		}
		return false;
